...
2. **Clip generation** (`cut_clips_random.ps1` / `cut_clips_10s.ps1`): scans one or more source folders for `*.mp4`, and in a loop until a target total duration is reached: picks a source video (weighted by folder in the `_random` variant), picks a random in-bounds start offset (`ffprobe` for duration, respecting `$skipStart`/`$skipEnd`), cuts a clip via `ffmpeg` (scaled/padded to `$width`x`$height`@`$fps`), and appends it to a running list. `cut_clips_random.ps1` additionally rolls a random clip *type* per cut (4/8/16/32 beats at `$bpm`, via `Get-RandomClipType`) and a random source *folder* per cut (via `Get-RandomFolder`, weighted by `$sourceFolders[].weight`). Clips are written to a `clips_*` subfolder, listed in an ffmpeg concat file, then concatenated (stream copy) into `final_mix*.mp4`.
